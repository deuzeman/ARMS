...
double Comparator::averages(Point const &point)
{
  ...
  d_error = 1.0;
  d_disc.clear();
  while ((d_error > d_prec) && samples < d_sampMax)
  {
    if (Log::ionode)
    ...
    needed = std::min(std::max(roundToBlocks(static_cast< unsigned long >(std::pow((d_error / d_prec), 2.0) * samples)), 1000ul), 50000ul);
    if (d_error < d_prec && Log::ionode)
      log() << "  >>  This is sufficient for the currently needed precision.\n" << std::endl;
  }
  if (samples >= d_sampMax)
    if (Log::ionode)
      log() << " >> [WARNING] Sample number exceeding maximum!\n" << std::endl;
  return d_result;
}
